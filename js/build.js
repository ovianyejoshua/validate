...
async function generateBlueprint() {
  ...
  try {
    const text = await callClaude(BLUEPRINT_SYSTEM + CONCISE, [{ role: 'user', content: buildContext() + '\n\nGenerate the complete product blueprint as a JSON object.' }], false, 5000, OPUS);
    clearInterval(bpInt);
...
Avoid: ${chapterBrief.avoid}`;
  ...
  try {
    const text = await callClaude(CHAPTER_SYSTEM, [{ role: 'user', content: userMsg }], false, 7000, SONNET);
    clearInterval(chInt);
